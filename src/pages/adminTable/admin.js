...
function AdminDash() {
							...
							{iconEdit && (
								<Button
									...
									onClick={() => {
										axios
											.get(
												`${process.env.REACT_APP_URL}/user/${tableMeta.rowData[0]}`
											)
											.then((response) => {
	...
	const EditData = () => {
		axios
			.put(`${process.env.REACT_APP_URL}/user/edit/${Id}`, DataEdit)
			.then((res) => {
				console.log(res);
				getData();
				console.log(res);
		...
		<>
					...
					{visibleEdit && (
						<form>
							...
							<TextField
								...
								onChange={handelChangeEdit}
							/>
							<label htmlFor="role"> PhoneNumber</label>
							<TextField
								type="text"
								name="phoneNumber"
								defaultValue={DataById.phoneNumber}
								onChange={handelChangeEdit}
							/>
							<label htmlFor="role"> Role</label>
							<TextField
            ...
            /> */}
				...
				<div className="income_table">
					<h3 className="pagetitle">Admins</h3>
